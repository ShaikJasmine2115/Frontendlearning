...
{
    if(!products.find((j)=> (j.id==pid && j.Item==pname)))
    {
        let x={};
        x.id=pid;
        x.Item=pname;
...
function updateprice(pdid,pdname,pdprice)
{
    const pdtoupdate=products.find(x => x.id==pdid);
    if (pdtoupdate)
    {
        pdtoupdate.Item=pdname;
        pdtoupdate.price=pdprice;
        return true;
    }
    return false;
...
document.addEventListener("DOMContentLoaded", ()=> {
        ...
        displayall(viewall());
            ...
            adding.addEventListener("click", ()=>{
                ...
                displayadded(addproduct(pid,pname,pprice));    
         })
            ...
            updating.addEventListener("click", ()=>{
                ...
                displayupdated(updateprice(updatepid,updatepdname,updatepdprice));
        })
